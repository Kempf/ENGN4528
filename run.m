...
%         end
    ...
    for i = 1:size(bird_pixel,1)
        ...
        if ~isempty(sling_coord)
%             bird_coord(i,1) =  round(scale*(bird_pixel(i,1) - sling_coord(1)));
%             bird_coord(i,2) = round(scale*(sling_coord(2) - bird_pixel(i,2)));
            bird_coord(i,3) = 1;
        end
        ...
        if bird_coord(i,3) == 1  && bird_coord(i,2) >30
            bird_coord(i,3) = 2;
        end
        
        if bird_coord(i,3) == 2  || bird_coord(i,3) == 0
            if bird_coord(i,1) > sling_coord(1)
                bird_coord(i,3) = 3;
    ...
    index=find(bird_coord(:,3) == 3);
    if isempty(index)~=1
        m=m*tform;
        coord_trajectory=[coord_trajectory;[bird_coord(index,1:2),1]/m];
